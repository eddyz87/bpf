...
static void bpf_diag_record_caller_saved(struct bpf_verifier_env *env, struct bpf_reg_state *regs)
{
	int i;

	for (i = 1; i < CALLER_SAVED_REGS; i++)
		bpf_diag_record_scrub(env, &regs[caller_saved[i]], BPF_DIAG_MOD_CALLER_SAVED);
}
			  ...
			  bool allow_trust_mismatch, const char *ctx)
{
	...
	/* Note: bpf_diag_mod_begin's reason might get refined by check_stack_read_fixed_off() */
	...
	while ((id = idstack_pop(idstack))) {
		...
		bpf_for_each_reg_in_vstate_mask(
			vstate, state, reg, stack, mask, ({
				...
				/*
				 * A dynptr occupies two stack slots that a single
				 * invalidate_dynptr() clears at once; record both
				 * scrubs before it runs.
				 */
