...
static void mark_current_stack_write(struct bpf_verifier_env *env, u32 insn_idx, u64 mask)
{
...
static void mark_stack_write(struct bpf_verifier_env *env,
			     struct bpf_verifier_state *st,
			     u32 frame,
			     u64 mask)
{
	u32 reg_frame_ip = frame_insn_idx(st, frame);

	if (frame == st->curframe)
		mark_current_stack_write(env, reg_frame_ip, mask);
	else
				   ...
				   enum bpf_arg_type arg_type, int insn_idx, int clone_ref_obj_id)
{
	...
	mark_stack_write(env, env->cur_state, state->frameno, gen_mask(spi - 1, spi));
				 ...
				 struct btf *btf, u32 btf_id, int nr_slots)
{
	...
	mark_stack_write(env, env->cur_state, state->frameno, gen_mask(spi - nr_slots + 1, spi));
				     ...
				     int kfunc_class)
{
	...
	mark_stack_write(env, env->cur_state, reg->frameno, BIT(spi));
	__mark_reg_known_zero(st);
	st->type = PTR_TO_STACK; /* we don't have dedicated reg type */
	st->live |= REG_LIVE_WRITTEN;
	st->ref_obj_id = id;
				      ...
				      int off, int size, int dst_regno)
{
	...
	mark_stack_read(env, frame_insn_idx(env->cur_state, reg_state->frameno), BIT(spi));
...
mark:
		...
		mark_reg_read(env, &state->stack[spi].spilled_ptr,
			      ...
			      REG_LIVE_READ64);
		mark_stack_read(env, frame_insn_idx(env->cur_state, reg->frameno), BIT(spi));
			     ...
			     u32 ip)
{
	...
	for (i = 0; i < st->allocated_stack / BPF_REG_SIZE; i++) {
		if (!(aux->live_stack_before & BIT(i))) {
