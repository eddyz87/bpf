 ...
 * in the paper "A New Algorithm for Identifying Loops in Decompilation"
 * by Tao Wei, Jian Mao, Wei Zou and Yu Chen, link:
 * https://api.semanticscholar.org/CorpusID:15784067
 *
 * Verifier only needs a link to topmost loop entry, thus the logic to
 * track nested loops structure is dropped.
 * Instead of CFG verifier operates on states graph.
 *
					     ...
					     bool speculative)
{
	...
	if (elem->st.parent) {
		elem->st.dfs_depth = elem->st.parent->dfs_depth + 1;
...
skip_inf_loop_check:
		 ...
		 * See comment before get_loop_entry().
		 */
