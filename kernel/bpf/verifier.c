				...
				u8 opcode, bool is_jmp32)
{
	...
	case BPF_JSLT:
		if (is_jmp32) {
			lo32 = cnum32_from_srange(S32_MIN, reg_s32_max(reg2) - 1);
			hi32 = cnum32_from_srange(reg_s32_min(reg1) + 1, S32_MAX);
			reg1->r32 = cnum32_intersect(reg1->r32, lo32);
			reg2->r32 = cnum32_intersect(reg2->r32, hi32);
		} else {
			lo = cnum64_from_srange(S64_MIN, reg_smax(reg2) - 1);
			hi = cnum64_from_srange(reg_smin(reg1) + 1, S64_MAX);
			reg1->r64 = cnum64_intersect(reg1->r64, lo);
			reg2->r64 = cnum64_intersect(reg2->r64, hi);
		}
		break;
