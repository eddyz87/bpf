		      ...
		      u16 base, u16 step)
{
	...
	fake_reg->umin_value = 0;
	fake_reg->umax_value = amount;
...
{
	u64 umin = max(tgt_min, reg->umin_value);
	u64 umax = min(tgt_max, reg->umax_value);
...
static bool at_terminating_loop_latch(struct bpf_verifier_env *env, struct bpf_verifier_state *st)
{
	struct bpf_insn_aux_data *aux = env->insn_aux_data;
...
static int compute_postorder(struct bpf_verifier_env *env)
{
	...
	preorder_nums = kvzalloc_objs(int, env->prog->len, GFP_KERNEL_ACCOUNT);
	postorder = kvzalloc_objs(int, env->prog->len, GFP_KERNEL_ACCOUNT);
	preorder = kvzalloc_objs(int, env->prog->len, GFP_KERNEL_ACCOUNT);
...
static int is_state_visited(struct bpf_verifier_env *env, int insn_idx)
{
	...
	env->cache_miss_frame = -2;
	env->cache_miss_reg = -2;
	env->cache_miss_spi = -2;
