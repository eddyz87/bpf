 ...
 */
static void find_equal_scalars(struct bpf_verifier_env *env,
			       struct bpf_verifier_state *vstate,
			       u32 id, u64 *equal_scalars)
{
	struct bpf_func_state *func;
	struct bpf_reg_state *reg;
	int i, j;
			     ...
			     struct bpf_insn *insn, int *insn_idx)
{
	...
	if (BPF_SRC(insn->code) == BPF_X && src_reg->type == SCALAR_VALUE && src_reg->id)
		find_equal_scalars(env, this_branch, src_reg->id, &equal_scalars);
	if (dst_reg->type == SCALAR_VALUE && dst_reg->id)
		find_equal_scalars(env, this_branch, dst_reg->id, &equal_scalars);
