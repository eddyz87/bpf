...
static bool eval_expr(struct bpf_verifier_env *env, struct scev *scev, struct bpf_func_state *st, u32 id, u64 *result)
{
	...
	if (is_reg(scev, id, &regno)) {
		...
		if (!reg) {
			bpf_log(&env->log, "scev_regno_to_reg(%d) -> NULL\n", regno);
			return false;
		}
...
{
	if (!reg
	    || (reg->type & PTR_MAYBE_NULL)	/* arithmetic on maybe-null is rejected */
	    || (reg->type & PTR_UNTRUSTED))	/* untrusted mem isn't offset-tracked */
		return false;
 ...
 */
static bool linear_bounds(struct bpf_reg_state *reg, struct bpf_loop_iters *iters,
			  s64 slope_imm, struct bounds *out)
{
	u32 alignment = min_t(u32, tnum_alignment(reg->var_off), __ffs(slope_imm));
	s64 min_val = reg_smin(reg);
	s64 max_val = reg_smax(reg);
	s64 total_change;

	if (check_mul_overflow(slope_imm, (s64)iters->max_header_count - 1, &total_change))
		return false;
	if (slope_imm > 0) {
			...
			struct bpf_loop_iters *iters)
{
	...
	for (r = 0; r < REGS_NUM; r++) {
		...
		//bpf_log(log, "bpf_widen_scev_regs: r=%d alive\n", r);
		...
		if (is_reg(scev, r_scev, &ra) && r == ra) {
			//bpf_log(log, "bpf_widen_scev_regs: r=%d is_reg\n", r);
			continue;
		}

		/* (any 1 (any 2 (any 3 4))) */
		if (is_any_imm_reg(scev, r_scev)) {
			//bpf_log(log, "bpf_widen_scev_regs: r=%d is_any_imm_reg\n", r);
			continue;
		}
...
cant_widen:
	...
	for (r = 0; r < REGS_NUM; r++) {
		...
		if (is_simple_linear(scev, r_expr, &base_reg, &slope_imm) &&
		    base_reg == r) {
			reg = scev_regno_to_reg(cur_func, r);
			...
			if (log->level & BPF_LOG_LEVEL2) {
				...
				log_reg(env, r);
				bpf_log(log, " to %lld..%lld alignment %u\n",
					cnum64_smin(bounds.range), cnum64_smax(bounds.range),
					bounds.alignment);
			}
			err = bpf_set_reg_range(env, reg, bounds.range, bounds.alignment);
			if (err)
				return err;
			...
			struct bpf_verifier_state *entry_state, struct bpf_loop_iters *iters)
{
	...
	for (r = 0; r < REGS_NUM; r++) {
		...
		if (!entry_reg) {
			bpf_log(&env->log, "scev_regno_to_reg(%d) -> NULL [entry_st]\n", r);
			continue;
		}
		reg = scev_regno_to_reg(cur_func_state, r);
		if (!reg) {
			bpf_log(&env->log, "scev_regno_to_reg(%d) -> NULL [cur_func_st]\n", r);
			continue;
		}
