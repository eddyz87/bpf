...
static int assign_header(struct bpf_verifier_env *env, struct loops_dfs *dfs, int n, int h)
{
	...
	/*
	 * bpf_log(&env->log, "assign_header(n=%d, h=%d)\n", n, h);
	 */
...
static int find_dominating_condition(struct bpf_verifier_env *env, int n, int top)
{
	...
	while (n >= 0) {
		if (is_cond_jmp_insn(&insns[n]) &&
		    bpf_loop_at_index(env, n) == nloop &&
		    (bpf_loop_at_index(env, n + 1) != nloop ||
		     bpf_loop_at_index(env, n + insns[n].off + 1) != nloop))
			return n;
		if (n == top)
			break;
		n = idoms[n];
...
static int compute_loops_in_subprog(struct bpf_verifier_env *env, struct loops_dfs *dfs, int subprog_idx)
{
	...
	do {
		...
		/*
		 * bpf_log(&env->log, "cur=%d, next_succ=%d, s=%d\n",
		 * 	cur, state[cur].next_succ, state[cur].next_succ == succ->cnt ?
		 * 				   -1 : succ->items[state[cur].next_succ]);
		 */
