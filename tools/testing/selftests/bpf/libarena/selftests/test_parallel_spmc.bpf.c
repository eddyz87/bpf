...
static int spmc_common_init(u64 total)
{
	...
	for (i = 0; i < TEST_SPMC_MAX_VALUES && can_loop; i++)
		seen[i] = 0;
...
int spmc_quiesce_on_owner(u64 epoch)
{
	...
	//for (i = 0; i < TEST_SPMC_SYNC_SPINS && can_loop; i++) {
...
static int spmc_singleval_owner(void)
{
	...
	for (i = 0; i < TEST_SPMC_SINGLEVAL_ITERS && can_loop; i++) {
...
int spmc_wait_for_stealers_to_start(u64 target)
{
	...
	bpf_for(spins, 0, TEST_SPMC_SYNC_SPINS) {
		if (test_abort)
			return -EINTR;
...
void spmc_waste_time(void)
{
	...
	for (i = 0; i < TEST_SPMC_WASTE_ROUNDS && can_loop; i++) {
		/* Random computation. */
...
int spmc_wait_for_steals(u64 target)
{
	...
	for (spins = 0; spins < TEST_SPMC_SYNC_SPINS && can_loop; spins++) {
		if (test_abort)
			return -EINTR;
...
static int spmc_resize_owner(void)
{
	...
	for (i = 0; i < TEST_SPMC_RESIZE_PREFILL && can_loop; i++) {
		ret = spmc_owned_add(spmc, i);
		if (ret) {
			test_abort = true;
			return ret;
...
spmc_burst_owner_round(u64 round)
{
	...
	for (i = 0; i < TEST_SPMC_BURST_BURST && can_loop; i++) {
	...
	for (i = 0; i < TEST_SPMC_BURST_BURST - stolen && can_loop; i++) {
		ret = spmc_owned_remove(spmc, &val);
		if (ret)
			return ret;
...
static int spmc_burst_owner(void)
{
	...
	for (round = 0; round < TEST_SPMC_BURST_ROUNDS && can_loop; round++) {
