...
struct btf_dump {
	...
	struct btf_dump_queue_item *emit_queue;
	int emit_queue_cap;
	int emit_queue_cnt;
...
int btf_dump__dump_type(struct btf_dump *d, __u32 id)
{
	...
	for (i = 0; i < d->emit_queue_cnt; i++) {
		fwd = d->emit_queue[i].flags & BTF_DUMP_FWD_DECL;
		LIBBPF_OPTS_RESET(opts, .fwd = fwd);
		err = btf_dump__emit_type_def(d, d->emit_queue[i].id, &opts);
		if (err < 0)
			return libbpf_err(err);
		if (err > 0)
			btf_dump_printf(d, ";\n\n");
...
struct btf_dump_queue_item *btf_dump__get_emit_queue(struct btf_dump *d, __u32 *cnt)
{
	*cnt = d->emit_queue_cnt;
	return d->emit_queue;
}
...
static struct missing_alias *btf_dump_find_missing_alias(struct btf_dump *d, __u32 id);
static bool btf_dump_is_blacklisted(struct btf_dump *d, __u32 id);

static int btf_dump_add_emit_queue_id(struct btf_dump *d, __u32 id, __u32 flags)
{
	struct btf_dump_queue_item *new_queue = NULL;
	size_t new_cap;
	...
	d->emit_queue[d->emit_queue_cnt].flags = flags;
	d->emit_queue_cnt++;
	return 0;
}
...
static int btf_dump_add_emit_queue_fwd(struct btf_dump *d, __u32 id)
{
	...
	return btf_dump_add_emit_queue_id(d, id, BTF_DUMP_FWD_DECL);
}
...
static int btf_dump_order_type(struct btf_dump *d, __u32 id, __u32 cont_id, bool through_ptr)
{
	...
	case BTF_KIND_INT:
		...
		if (btf_dump_find_missing_alias(d, id))
			return btf_dump_add_emit_queue_id(d, id, 0);
		else
			return 0;
	case BTF_KIND_FLOAT:
		tstate->order_state = ORDERED;
	...
	case BTF_KIND_UNION: {
		...
		vlen = btf_vlen(t);
		for (i = 0; i < vlen; i++, m++) {
			err = btf_dump_order_type(d, m->type, t->name_off == 0 ? cont_id : id,
						  false);
			if (err < 0)
				return err;
		}
		...
		if (t->name_off != 0) {
			err = btf_dump_add_emit_queue_id(d, id, 0);
			if (err < 0)
				return err;
		}
...
{
	return libbpf_err(btf_dump_order_type(d, id, id, false));
}

static int btf_dump_emit_missing_aliases(struct btf_dump *d, __u32 id);
 ...
 */
static int btf_dump_emit_type_def(struct btf_dump *d, __u32 id,
...
int btf_dump__emit_type_def(struct btf_dump *d, __u32 id,
			    struct btf_dump_emit_type_opts *opts)
{
	return libbpf_err(btf_dump_emit_type_def(d, id, opts));
}
...
struct missing_alias {
	const char *name;
	const char *text;
};

static struct missing_alias missing_base_types[] = {
	/*
...
static struct missing_alias *btf_dump_find_missing_alias(struct btf_dump *d, __u32 id)
{
	const char *name = btf_name_of(d, id);
	int i;

	for (i = 0; i < ARRAY_SIZE(missing_base_types); i++) {
		if (strcmp(name, missing_base_types[i].name) == 0)
			return &missing_base_types[i];
	}
	return NULL;
}
...
static int btf_dump_emit_missing_aliases(struct btf_dump *d, __u32 id)
{
	struct missing_alias *alias;

	alias = btf_dump_find_missing_alias(d, id);
	if (!alias)
		return 0;

	btf_dump_printf(d, "typedef %s %s", alias->text, alias->name);
	return 1;
}
